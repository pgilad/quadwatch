...
release_date="$(TZ=Asia/Jerusalem date +%Y.%m.%d)"
...
if [[ -n "${previous_tag}" ]]; then
  ...
  changelog_entries="$(git log --no-merges --reverse --format='- %s (%h)' "${revision_range}" -- . ':(exclude)CHANGELOG.md' || true)"
else
  changelog_entries="$(git log --no-merges --reverse --format='- %s (%h)' HEAD -- . ':(exclude)CHANGELOG.md' || true)"
fi
